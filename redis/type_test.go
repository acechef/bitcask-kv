...
func TestRedisDataStructure_Del_Type(t *testing.T) {
	...
	err = rds.Delete(utils.GetTestKey(11))
	assert.Nil(t, err)
...
func TestRedisDataStructure_HGet(t *testing.T) {
	...
	var1, err := rds.HGet(utils.GetTestKey(1), []byte("field1"))
	assert.Nil(t, err)
	assert.Equal(t, v1, var1)
	var2, err := rds.HGet(utils.GetTestKey(1), []byte("field2"))
	assert.Nil(t, err)
	assert.Equal(t, v2, var2)
...
func TestRedisDataStructure_ZScore(t *testing.T) {
	...
	ok, err := rds.ZAdd(utils.GetTestKey(1), 113, []byte("value-1"))
	assert.Nil(t, err)
	assert.True(t, ok)
	ok, err = rds.ZAdd(utils.GetTestKey(1), 333, []byte("value-1"))
	assert.Nil(t, err)
	assert.False(t, ok)
	ok, err = rds.ZAdd(utils.GetTestKey(1), 98, []byte("value-2"))
	assert.Nil(t, err)
	assert.True(t, ok)

	score, err := rds.ZScore(utils.GetTestKey(1), []byte("value-1"))
	assert.Nil(t, err)
	assert.Equal(t, score, float64(333))
	score, err = rds.ZScore(utils.GetTestKey(1), []byte("value-2"))
	assert.Nil(t, err)
	assert.Equal(t, score, float64(98))
}
